...
/*------THIS CODE IS NOT PART OF OCORNUT'S IMGUI !!!!---------------------------------------------------*/
/*------IT WAS ADDED BY JULES FOUCHY TO RENDER IMAGES INSIDE TABS INSTEAD OF TEXT-----------------------*/
/*------THIS IS ESSENTIALY A COPY-PASTE OF THE ABOVE CODE, WITH SLIGHT CHANGES TO INTRODUCE IMAGES------*/

bool ImGui::BeginTabItemWithImage(ImTextureID texture, ImVec2 imageSizeInTab, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
...
bool ImGui::TabItemImageAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, ImTextureID texture, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    if (bb.GetWidth() <= 1.0f)

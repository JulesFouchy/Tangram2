...
unsigned int Settings::m_SELECT_SAFETY__MIN_TIME_BEFORE_MOVING_LAYER_IN_MS = 115;
...
void Settings::ImGuiParameters() {
	...
	ImGui::SliderFloat("Min Opacity to Grab Layer", &m_MIN_OPACITY_TO_GRAB_LAYER, 0.0f, 255.0f); 
	{
		...
		ImGui::SliderScalar("Selection Safety : Min Time in ms before Translating a Layer", ImGuiDataType_U32, &m_SELECT_SAFETY__MIN_TIME_BEFORE_MOVING_LAYER_IN_MS, &m, &M);
	}
}